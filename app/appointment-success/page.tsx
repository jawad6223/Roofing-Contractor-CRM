...
function SuccessContent() {
  ...
  useEffect(() => {
    const verifyAndCreate = async () => {
      ...
      const res = await fetch(`/api/verify-appointment-session?session_id=${sessionId}`);
      const { paid, error } = await res.json();

      if (paid && !error) {
        console.log("✅ Payment confirmed, appointment request created");
        setStatus("success");
      } else {
        setStatus("failed");
      }
        ...
        {status === "success" && (
          <>
            ...
            <p className="text-gray-600 mb-6">
              Your appointment request has been created successfully. We'll process it shortly.
            </p>
            <Button
